...
const Bathroom = () =>{
    ...
    const handleDetails = (slug:string) =>{
        router.push(`bathroom/${slug}`)
      } 
    return(
        <section className="py-5 md:py-10">
            <MaxWidthWrapper>
            <div className="flex flex-col md:flex-row justify-between">
                <h1 className="text-lg md:text-2xl font-semibold text-black capitalize">Keep Shopping For Bathroom Storage</h1>
                ...
                {cardData?.map((card)=> (
                        <CarouselItem className=" basis-full sm:basis-1/2 md:basis-1/3 lg:basis-1/5 mt-8" key={card.id}>
                            ...
                            <div className="p-3 rounded-lg border shadow-sm">
                                ...
                                <p className="text-base font-bold text-start flex gap-2">{card.price} <del className="text-gray-400">{card.oldPrice}</del></p>
