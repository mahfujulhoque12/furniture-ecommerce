...
import { cardData } from "@/data/multiFunData";
import Image from "next/image";
import React from 'react'
import { useRouter } from "next/navigation";

const MultiFun:React.FC = () => {

    const router = useRouter()
    const handleDetails = (slug:string) => {
        router.push(`multi-functional-furniture/${slug}`)
    }
  ...
  return (
    <section className="py-5 md:py-10">
        ...
        <div>
            <h1 className="text-lg text-start  sm:text-start  md:text-2xl font-semibold text-black capitalize">Multi Functional Furniture</h1>
