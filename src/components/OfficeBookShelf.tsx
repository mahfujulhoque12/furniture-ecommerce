...
const OfficeBookShelf = () =>{
    const router = useRouter()

    const handleDetails = (slug:string) =>{
      router.push(`office-furniture/${slug}`)
    }
    const chairData = cardData.filter((card)=> card.id >=551 && card.id <= 560)
    return(
        <section className="py-5 md:py-10">
            <MaxWidthWrapper>
            <div className="flex flex-col md:flex-row justify-between">
                <h1 className="text-lg text-center  sm:text-start text-center  sm:text-start md:text-2xl font-semibold text-black capitalize">Office Book Shelf</h1>
                    ...
                    >
                        <div  onClick={() => handleDetails(card.slug)} className="p-3 rounded-lg border shadow-sm h-full flex flex-col cursor-pointer ">
                                <div className="transition-all duration-300 hover:scale-[101%]">
                                    ...
                                    <p className="text-sm text-start font-normal mb-1">
                                        {card.des.slice(0,58)}...</p>
...
export default OfficeBookShelf;