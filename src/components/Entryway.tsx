...
import { cardData } from "@/data/entrywayData";
import Image from "next/image";
import React from 'react'
import { useRouter } from "next/navigation";

const Entryway:React.FC = () => {

    const router = useRouter()
    const handleDetails = (slug:string) => {
        router.push(`entryway-furniture/${slug}`)
    }
  ...
  return (
    <section className="py-5 md:py-10">
        ...
        <div>
            <h1 className="text-lg text-start  sm:text-start  md:text-2xl font-semibold text-black capitalize">Entryway Furniture</h1>
